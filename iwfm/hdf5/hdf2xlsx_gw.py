...
def hdf2xlsx_gw(hdf_file, output_file,
    ...
    with h5py.File(hdf_file, 'r') as f:
        ...
        for loc_idx in range(n_locations):
            ...
            budget_title = f"GROUNDWATER BUDGET IN {vol_label} FOR {loc_name}"
            ws['A2'] = budget_title
            ws['A2'].font = Font(bold=True)

            if area > 0:
                area_title = f"SUBREGION AREA: {area:,.2f} {area_label}"
                ws['A3'] = area_title
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Convert IWFM Groundwater Budget HDF5 file to Excel workbook',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Basic usage with defaults (converts to acres and acre-feet)
  python hdf2xlsx_gw.py C2VSimFG_GW_Budget.hdf output.xlsx

  # Specify custom conversion factors
  python hdf2xlsx_gw.py input.hdf output.xlsx --area-fact 0.000022957 --vol-fact 0.000022957

  # Use different units
  python hdf2xlsx_gw.py input.hdf output.xlsx --area-units "hectares" --vol-units "cubic meters"
...
Default conversion factors:
  ...
  Volume: 0.000022957 (cu ft to acre-ft, exact: 1/43560)
    ...
    if args.hdf_file:
        ...
        # Interactive mode
        print("IWFM Groundwater Budget HDF5 to Excel Converter")
        hdf_file = input('  Enter Groundwater HDF5 budget file name: ')
