...
def hdf2xlsx_snodes(hdf_file, output_file,
    ...
    - Sheet1 (empty)
    - One sheet per stream node with budget data
    ...
    with h5py.File(hdf_file, 'r') as f:
        ...
        for loc_idx in range(n_locations):
            ...
            budget_title = f"STREAM NODE BUDGET IN {vol_label} FOR {loc_name}"
            ws['A2'] = budget_title
            ws['A2'].font = Font(bold=True)

            # Stream nodes don't have areas, so skip the area title row
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Convert IWFM Stream Node Budget HDF5 file to Excel workbook',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Basic usage with defaults (converts to acres and acre-feet)
  python hdf2xlsx_snodes.py C2VSimFG_Stream_NodeBudget.hdf output.xlsx

  # Specify custom conversion factors
  python hdf2xlsx_snodes.py input.hdf output.xlsx --area-fact 0.000022957 --vol-fact 0.000022957

  # Use different units
  python hdf2xlsx_snodes.py input.hdf output.xlsx --area-units "hectares" --vol-units "cubic meters"
...
Notes:
  ...
  Unlike stream reach budgets, node budgets do not include area information.
    ...
    if args.hdf_file:
        ...
        # Interactive mode
        print("IWFM Stream Node Budget HDF5 to Excel Converter")
