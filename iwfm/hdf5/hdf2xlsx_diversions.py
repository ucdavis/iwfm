...
def hdf2xlsx_diversions(hdf_file, output_file,
    ...
    - Sheet1 (empty)
    - One sheet per location with diversions data
    ...
    with h5py.File(hdf_file, 'r') as f:
        ...
        for loc_idx in range(n_locations):
            ...
            budget_title = f"STREAM DIVERSIONS IN {vol_label} FOR {loc_name}"
            ws['A2'] = budget_title
            ws['A2'].font = Font(bold=True)

            if area > 0:
                area_title = f"DIVERSION AREA: {area:.2f} {area_label}"
                ws['A3'] = area_title
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Convert IWFM Stream Diversions HDF5 file to Excel workbook',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Basic usage with defaults (converts to acres and acre-feet)
  python hdf2xlsx_diversions.py C2VSimFG_Stream_Diversions.hdf output.xlsx

  # Specify custom conversion factors
  python hdf2xlsx_diversions.py input.hdf output.xlsx --area-fact 0.000022957 --vol-fact 0.000022957

  # Use different units
  python hdf2xlsx_diversions.py input.hdf output.xlsx --area-units "hectares" --vol-units "cubic meters"
...
Default conversion factors:
  ...
  Volume: 0.000022957 (cu ft to acre-ft, exact: 1/43560)
    ...
    if args.hdf_file:
        ...
        # Interactive mode
        print("IWFM Stream Diversions HDF5 to Excel Converter")
