...
def hdf2xlsx_rz(hdf_file, output_file,
    ...
    with h5py.File(hdf_file, 'r') as f:
        ...
        for loc_idx in range(n_locations):
            ...
            budget_title = f"ROOT ZONE BUDGET IN {vol_label} FOR {loc_name}"
            ws['A2'] = budget_title
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Convert IWFM Root Zone Budget HDF5 file to Excel workbook',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Basic usage with defaults (converts to acres and acre-feet)
  python hdf2xlsx_rz.py C2VSimFG_RZ_Budget.hdf output.xlsx

  # Specify custom conversion factors
  python hdf2xlsx_rz.py input.hdf output.xlsx --area-fact 0.000022957 --vol-fact 0.000022957

  # Use different units
  python hdf2xlsx_rz.py input.hdf output.xlsx --area-units "hectares" --vol-units "cubic meters"
...
Default conversion factors:
  ...
  Volume: 0.000022957 (cu ft to acre-ft, exact: 1/43560)
    ...
    if args.hdf_file:
        ...
        # Interactive mode
        print("IWFM Root Zone Budget HDF5 to Excel Converter")
        hdf_file = input('  Enter Root Zone HDF5 budget file name: ')
