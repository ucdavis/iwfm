...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Convert IWFM Stream Diversions HDF5 file to text format',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Basic usage with defaults (converts to acres and acre-feet)
  python hdf2bud_diversions.py C2VSimFG_Stream_Diversions.hdf output.txt

  # Specify custom conversion factors
  python hdf2bud_diversions.py input.hdf output.txt --area-fact 0.000022957 --vol-fact 0.000022957

  # Use different units
  python hdf2bud_diversions.py input.hdf output.txt --area-units "hectares" --vol-units "cubic meters"
...
Default conversion factors:
  ...
  Volume: 0.000022957 (cu ft to acre-ft, exact: 1/43560)
    ...
    parser.add_argument('--len-fact', type=float, default=1.0, help='Length conversion factor (default: 1.0)')
    parser.add_argument('--len-units', type=str, default='FEET', help='Length units for output (default: FEET)')
    ...
    parser.add_argument('--area-units', type=str, default='AC', help='Area units for output (default: AC)')
    ...
    parser.add_argument('--vol-units', type=str, default='ACFT', help='Volume units for output (default: ACFT)')
    ...
    if args.hdf_file:
        ...
        # Interactive mode
        print("IWFM Stream Diversions HDF5 to Text Converter")
        hdf_file = input('  Enter Stream Diversions HDF5 file name: ')
