...
def las2shp(source, target, debug=0):
    ...
    Parameters:
      ...
      target          (str):  Name of output shapefile
      debug           (int):  Turn on CLI priniting for debugging
    ...
    ** This function isn't working for the example LIDAR file **
    ...
    for i in range(tris):
        ...
        if percent % 10.0 == 0 and percent > last_percent:
            ...
            if debug:
                print(
                    "        {} % done - Shape {}/{} at {}".format(
                        percent, i, tris, time.asctime()
                    )
                )
        part = []
        x1 = las.x[t[0]]
