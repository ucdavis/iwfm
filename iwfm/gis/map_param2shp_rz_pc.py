...
def map_param2shp_rz_pc(param_types, param_vals, crops, elem_shp_name, out_shp_name='elem_parameters', verbose=False):
    ''' map_param2shp_rz_pc() - Read a shapefile of IWFM model elements and map IWFM 
                    Ponded Crop parameters to the elements
...
if __name__ == "__main__":
    ''' Run map_param2shp_rz_pc() from command line'''
    import sys
    ...
    param_types = ["cn", "et", "wsp", "ip", "pd", "ad", "rf", "ru", "ic"]

    crops, param_vals, files = iwfm.iwfm_read_rz_pc(rz_file_name)                 # Read rootzone parameters

    map_param2shp_rz_pc(param_types, param_vals, crops, elem_shp_name, out_shp_name=out_shp_name, verbose=verbose)
