...
def idw(x, y, elem, nnodes, nlayers, nodexy, elevations, debug=0):
    ...
    Parameters:
      x               (list): X values of points
      y               (list): Y values of points
      elem            (list): List of model elements and nodes
      nnodes          (list): List of model nodes
      nlayers         (int):  Number of model layers
      nodexy          (list): Spatial locations of nodes
      elevations      (list): Rop row of table data in excel spreadsheets
      debug           (int):  1 == Turn on debug printing
