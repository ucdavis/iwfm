...
class TestInverseDict:
    """Test the inverse_dict function."""
    
    def test_inverse_dict_basic(self):
        """Test basic dictionary inversion."""
        d = {'a': 1, 'b': 2, 'c': 3}
        result = iwfm.inverse_dict(d)
        expected = {1: 'a', 2: 'b', 3: 'c'}
        assert result == expected
    
    def test_inverse_dict_string_values(self):
        """Test inversion with string values."""
        d = {1: 'apple', 2: 'banana', 3: 'cherry'}
        result = iwfm.inverse_dict(d)
        expected = {'apple': 1, 'banana': 2, 'cherry': 3}
        assert result == expected
    
    def test_inverse_dict_mixed_types(self):
        """Test inversion with mixed key/value types."""
        d = {'key1': 100, 'key2': 200.5, 'key3': 'value'}
        result = iwfm.inverse_dict(d)
        expected = {100: 'key1', 200.5: 'key2', 'value': 'key3'}
        assert result == expected
    
    def test_inverse_dict_empty(self):
        """Test inversion of empty dictionary."""
        d = {}
        result = iwfm.inverse_dict(d)
        assert result == {}
    
    def test_inverse_dict_single_item(self):
        """Test inversion with single item."""
        d = {'only_key': 'only_value'}
        result = iwfm.inverse_dict(d)
        expected = {'only_value': 'only_key'}
        assert result == expected


class TestList2Dict:
    """Test the list2dict function."""
    
    def test_list2dict_basic(self):
        """Test basic list to dictionary conversion."""
        items = [['key1', 'val1'], ['key2', 'val2'], ['key3', 'val3']]
        result = iwfm.list2dict(items)
        expected = {'key1': ['val1'], 'key2': ['val2'], 'key3': ['val3']}
        assert result == expected
    
    def test_list2dict_multiple_values(self):
        """Test with multiple values per key."""
        items = [['key1', 'val1', 'val2'], ['key2', 'val3', 'val4', 'val5']]
        result = iwfm.list2dict(items)
        expected = {'key1': ['val1', 'val2'], 'key2': ['val3', 'val4', 'val5']}
        assert result == expected
    
    def test_list2dict_single_key_only(self):
        """Test with only keys (no values)."""
        items = [['key1'], ['key2'], ['key3']]
        result = iwfm.list2dict(items)
        expected = {'key1': [], 'key2': [], 'key3': []}
        assert result == expected
    
    def test_list2dict_numeric_keys(self):
        """Test with numeric keys."""
        items = [[1, 'val1'], [2, 'val2'], [3, 'val3']]
        result = iwfm.list2dict(items)
        expected = {1: ['val1'], 2: ['val2'], 3: ['val3']}
        assert result == expected
    
    def test_list2dict_mixed_types(self):
        """Test with mixed data types."""
        items = [['str_key', 123, 45.6], [789, 'text', True]]
        result = iwfm.list2dict(items)
        expected = {'str_key': [123, 45.6], 789: ['text', True]}
        assert result == expected
    
    def test_list2dict_empty_list(self):
